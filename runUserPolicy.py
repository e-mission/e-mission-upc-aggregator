...
def reset_alg_json(f):
    with open(f, "w") as jsonFile:
        json.dump({"ae": 0, "rc": 0}, jsonFile)
...
def main ():
    ...
    reset_alg_json("emission/simulation/known_algs.json")
    requests.post (controller_addr + "/setup_networks")
    ret = subprocess.Popen (["./e-mission-py.bash", "emission/simulation/simulate_fake_users.py", str(int(num_users / 2)), str(num_trips)], cwd="./")
    ret.wait ()
    wipe_alg_json("emission/simulation/known_algs.json")
    ret = subprocess.Popen (["./e-mission-py.bash", "emission/simulation/simulate_fake_users.py", str(int(num_users / 2)), str(num_trips)], cwd="./")
    ret.wait ()
    for query_file in query_files:
        csv_file_name = "csvs/policy_half_alg_correctness_" + query_file.split("/")[1] + "_0.05_" + str(num_users) + "_" + str(num_trips) + ".csv"
        for _ in range(num_queries):
            requests.post (controller_addr + "/pause_all_queriers")
            requests.post (controller_addr + "/pause_all_clouds")
            ret = subprocess.Popen (["./e-mission-py.bash", "emission/net/ext_service/launch_aggregator.py", query_file, csv_file_name, str(num_users)], cwd="./")
